...
function worker(fetch) {
  ...
  vm.runInNewContext(source, {
    ...
    caches: { open: async () => cache, keys: async () => ["unrelated-cache", "tl-calendar-shell-v9", "tl-calendar-shell-v63"], delete: async (key) => deleted.push(key) }
  });
...
test("cached versioned resources do not make redundant background requests", async () => {
  ...
  const scriptPath=source.match(/"\.\/app\.js\?v=\d+"/)[0].slice(2,-1);
  sw.stored.set(`https://calendar.example${scriptPath}`, new Response("cached script"));
  assert.equal(await (await sw.request(scriptPath)).text(), "cached script");
});
...
test("activation removes only this application's previous cache", async () => {
  ...
  await work;
  assert.deepEqual(sw.deleted, ["tl-calendar-shell-v9", "tl-calendar-shell-v63"]);
});
