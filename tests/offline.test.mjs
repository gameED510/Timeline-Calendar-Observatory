...
function worker(fetch) {
  ...
  vm.runInNewContext(source, {
    ...
    caches: { open: async () => cache, keys: async () => ["unrelated-cache", "tl-calendar-shell-v9", "tl-calendar-shell-v65"], delete: async (key) => deleted.push(key) }
  });
...
test("activation removes only this application's previous cache", async () => {
  ...
  await work;
  assert.deepEqual(sw.deleted, ["tl-calendar-shell-v9", "tl-calendar-shell-v65"]);
});
