...
function worker(fetch) {
  ...
  vm.runInNewContext(source, {
    ...
    caches: { open: async () => cache, keys: async () => ["unrelated-cache", "tl-calendar-shell-v4", "tl-calendar-shell-v5"], delete: async (key) => deleted.push(key) }
  });
...
test("cached versioned resources do not make redundant background requests", async () => {
  ...
  sw.stored.set("https://calendar.example/app.js?v=5", new Response("cached script"));
  assert.equal(await (await sw.request("/app.js?v=5")).text(), "cached script");
});
...
test("activation removes only this application's previous cache", async () => {
  ...
  await work;
  assert.deepEqual(sw.deleted, ["tl-calendar-shell-v4"]);
});
