...
function worker(fetch) {
  ...
  vm.runInNewContext(source, {
    ...
    caches: { open: async () => cache, keys: async () => ["unrelated-cache", "tl-calendar-shell-v9", "tl-calendar-shell-v30"], delete: async (key) => deleted.push(key) }
  });
...
test("cached versioned resources do not make redundant background requests", async () => {
  ...
  sw.stored.set("https://calendar.example/app.js?v=23", new Response("cached script"));
  assert.equal(await (await sw.request("/app.js?v=23")).text(), "cached script");
});
