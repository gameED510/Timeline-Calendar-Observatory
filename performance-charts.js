(function (root) {
  ...
  async function render(host, records, snapshot, onMonth, selectedMonth) {
    destroy(); const own=generation;
    const endMonth=selectedMonth || new Date().toISOString().slice(0,7);
    const points=root.TLPerformance.chartPoints(records,endMonth,months);
    ...
    host.querySelectorAll('[data-mode]').forEach(button=>{
      ...
      button.onclick=()=>{mode=button.dataset.mode;render(host,records,snapshot,onMonth,endMonth);};
    });
    const range=host.querySelector('select');range.value=String(months);range.onchange=()=>{months=Number(range.value);render(host,records,snapshot,onMonth,endMonth);};
    const monthList=host.querySelector('.actual-chart-months');
    for (const point of points) {
      if (!point.snapshot) {
        const row=document.createElement('tr');
        [point.month,'—','未录入','—'].forEach(value=>{const cell=document.createElement('td');cell.textContent=value;row.append(cell);});
    ...
    if (!points.some(p=>p.snapshot)) { status.textContent='此时间范围尚无实际结算记录';canvas.parentElement.hidden=true;return; }
    canvas.textContent=points.map(p=>p.snapshot?`${p.month}：公式 ${money(p.snapshot.formula)}，实际 ${money(p.total)}`:`${p.month}：未录入`).join('；');
    try {
      ...
      const series=mode==='amount' ? [
        {label:'公式估算',data:points.map(p=>p.snapshot?.formula ?? null),borderColor:'#3785c5',backgroundColor:'#3785c5'},
      ...
      ] : [{label:mode==='percent'?'误差 / 公式估算':'公式估算 - 实际',data:points.map(p=>!p.snapshot?null:mode==='percent'?(p.snapshot.formula>0?(p.snapshot.formula-p.total)/p.snapshot.formula*100:null):p.snapshot.formula-p.total),backgroundColor:points.map(p=>p.snapshot?.formula>=p.total?'#d77567':'#299481'),borderRadius:4,maxBarThickness:36}];
      ...
      status.textContent=mode==='amount'?'缺失月份留空，不代表零收入':`正值为高估，负值为低估${mode==='percent'?'；公式估算为零的月份不计算比例':''}`;
