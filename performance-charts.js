(function (root) {
  ...
  async function render(host, records, snapshot, onMonth) {
    ...
    host.innerHTML='<div class="actual-chart-heading"><div class="segmented actual-chart-tabs"><button type="button" data-mode="amount">金额趋势</button><button type="button" data-mode="error">估算偏差</button></div><select aria-label="图表时间范围"><option value="6">近 6 个月</option><option value="12">近 12 个月</option></select></div><div class="actual-chart-canvas"><canvas role="img" aria-label="绩效金额与估算误差趋势"></canvas></div><p class="chart-status" role="status"></p><div class="actual-chart-months"></div><div class="actual-platform-mix"><h4>本月公式估算构成</h4></div>';
    ...
    try {
      ...
      ] : [{label:'公式估算 - 实际',data:points.map(p=>p.snapshot.formula-p.total),backgroundColor:points.map(p=>p.snapshot.formula>=p.total?'#d77567':'#299481'),borderRadius:4,maxBarThickness:36}];
      instance=new Chart(canvas,{type:mode==='amount'?'line':'bar',data:{labels:points.map(p=>p.month.slice(2)),datasets:series},options:{responsive:true,maintainAspectRatio:false,animation:matchMedia('(prefers-reduced-motion: reduce)').matches?false:{duration:350},interaction:{intersect:false,mode:'index'},
        ...
        plugins:{legend:{position:'bottom',labels:{color:ink,usePointStyle:true,boxWidth:8,padding:14,font:{size:11}}},tooltip:{callbacks:{label:item=>`${item.dataset.label}：¥${money(item.parsed.y)}`}}},
        scales:{x:{grid:{display:false},ticks:{color:ink,font:{size:11}}},y:{beginAtZero:true,grid:{color:line},ticks:{color:ink,maxTicksLimit:5,callback:v=>Math.abs(v)>=10000?`${Math.round(v/1000)/10}万`:money(v)}}},
        onClick:(_event,elements)=>{if(elements.length)onMonth(points[elements[0].index].month);}
      }});
      status.textContent=mode==='error'?'正值为高估，负值为低估':'';
